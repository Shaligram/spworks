...
def run():
    ...
    net[ 'server' ].cmd( 'ip route add 10.0.11.0/24 via 10.0.13.1')
    #net[ 'router' ].cmd( 'route add -net 11.0.14.0/24 dev router-eth3')
    info( '*** Add traffic classification rules to select different routing table\n' )
    net[ 'router' ].cmd( 'ip rule del from 10.0.10.0/24 lookup net1_table' )
    net[ 'router' ].cmd( 'ip rule add from 10.0.10.0/24 lookup net1_table' )
    net[ 'router' ].cmd( 'ip rule del from 10.0.11.0/24 lookup net2_table' )
    net[ 'router' ].cmd( 'ip rule add from 10.0.11.0/24 lookup net2_table' )
    print ( net[ 'router' ].cmd( 'ip rule list' ))
    print ( '*** Add routing Tables\n' )
    net[ 'router' ].cmd( 'grep net1_table /etc/iproute2/rt_tables || echo "101 net1_table">> /etc/iproute2/rt_tables' )
    net[ 'router' ].cmd( 'grep net2_table /etc/iproute2/rt_tables || echo "102 net2_table">> /etc/iproute2/rt_tables' )
    print ( net[ 'router' ].cmd( 'egrep net.*_table /etc/iproute2/rt_tables' ))
    print ( "*** Add routes to the routing tables\n" )
    net[ 'router' ].cmd( 'ip route add 11.0.14.0/24 via 10.0.12.2 table net1_table')
    net[ 'router' ].cmd( 'ip route add 11.0.14.0/24 via 10.0.13.2 table net2_table')
    print ( net[ 'router' ].cmd( 'ip route list table net1_table'))
    print ( net[ 'router' ].cmd( 'ip route list table net2_table'))
    print ( '*** Routing Table on Router\n' )
    print ( net[ 'router' ].cmd( 'route -n|grep router' ))
    print ( '*** Routing Table on Server\n' )
